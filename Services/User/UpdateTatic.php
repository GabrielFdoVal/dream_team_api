<?php
    ...
    function UpdateTatic($email, $tatic): ResultService {
        $result = new ResultService();

        if($email == null || $tatic == null){
            //Validação dos campos aqui, caso algum esteja errado o erro é 400.
            $result->Fail(400, "One or more fields are incorrect.");
            return $result;
        }

        include($_SERVER['DOCUMENT_ROOT']."/dream_team_api/Repositories/User/UpdateTatic.php");

        $code = ChangeTatic($email, $tatic);
