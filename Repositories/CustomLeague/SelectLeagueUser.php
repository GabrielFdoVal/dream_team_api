    ...
    function SelectLeagueUser($email) {
        ...
        try {
            ...
            $res_leagues = $DB->Consult(
                "SELECT DISTINCT c.cd_liga_comum, c.nm_liga_comum, c.qt_rodada, c.qt_usuario, c.ic_privada, c.cd_privada, o.qt_pontos, (SELECT u.nm_apelido
        ...
        if($res_leagues){
            $leagues = array();
            for($i = 0; $i < sizeof($res_leagues); $i = $i + 8){
                $league = new CustomLeague($res_leagues[$i], $res_leagues[$i+1], $res_leagues[$i+2], $res_leagues[$i+3], $res_leagues[$i+4], $res_leagues[$i+5], $res_leagues[$i+6], $res_leagues[$i+7]);
