    ...
    function SelectNotUpdated($match) {
        ...
        try {
            $res_player = $DB->Consult(
               ...
               //           and tb_partida.cd_rodada = tb_rodada.cd_rodada
               // where tb_rodada.cd_rodada = ".$match."
        ...
        if($res_player){
            $players = array(); 
            for($i = 0; $i < sizeof($res_player); $i = $i + 13){
                $player = new Player($res_player[$i],$res_player[$i+1],$res_player[$i+2],$res_player[$i+3],$res_player[$i+4],$res_player[$i+5],$res_player[$i+6],$res_player[$i+7],$res_player[$i+8],$res_player[$i+9],$res_player[$i+10],$res_player[$i+11],$res_player[$i+12]);
